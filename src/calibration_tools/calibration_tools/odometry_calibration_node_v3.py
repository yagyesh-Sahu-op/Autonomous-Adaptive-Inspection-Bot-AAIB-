...
"""
Robust Odometry Calibration Node with Timer-based Publishing
"""
...
class RobustCalibrationNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('rotation_angular_velocity', 0.5)  # Slower for better accuracy
        self.declare_parameter('linear_velocity', 0.2)
        self.declare_parameter('rotation_duration', 12.56)  # Longer duration at slower speed
        ...
        # Command publishing timer (50Hz)
        self.cmd_timer = self.create_timer(0.02, self.publish_command)
        ...
        self.get_logger().info('Robust Odometry Calibration Node Started')
        self.get_logger().info(f'Publishing commands at 50Hz on: {self.cmd_vel_topic}')
    ...
    def publish_command(self):
        """Timer callback to publish commands at consistent rate"""
    ...
    def rotation_test_callback(self, request, response):
        ...
        if self.current_pose is None:
            ...
            response.message = "No odometry data received! Check: ros2 topic echo " + self.odom_topic
        ...
        # Set rotation command and enable publishing
        self.current_command = Twist()
        self.current_command.angular.z = self.rotation_angular_vel
        self.publishing_enabled = True
        ...
        # Let robot settle and get final readings
        time.sleep(0.5)
        for _ in range(20):
